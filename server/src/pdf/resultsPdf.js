...
async function generateResultsPdf(roundId) {
  ...
  // Fetch used (counted) serial numbers with the candidate they voted for (from last complete pass)
  ...
  // Fetch audit entries for pass reopens and deletes
  const { rows: passAuditEntries } = await db.query(
    `SELECT entity_type, entity_id, from_status, to_status, changed_by, started_at
     FROM status_transitions
     WHERE entity_type IN ('pass_reopened', 'pass_deleted', 'round_reverse_finalize')
       AND entity_id IN (SELECT id FROM passes WHERE round_id = $1)
     UNION ALL
     SELECT entity_type, entity_id, from_status, to_status, changed_by, started_at
     FROM status_transitions
     WHERE entity_type IN ('round_reverse_finalize')
       AND entity_id = $1
     ORDER BY started_at`,
    [roundId]
  );
  ...
  // === 6. AUDIT LOG (Pass Reopens, Deletes, Reverse Finalizations) ===
  if (passAuditEntries.length > 0) {
    // Check if we need a new page
    if (doc.y > 550) doc.addPage();
    doc.fontSize(13).font('Helvetica-Bold').text('Audit Log');
    doc.moveDown(0.3);
    doc.fontSize(8).font('Helvetica-Bold');
    const aCol1 = 50, aCol2 = 170, aCol3 = 290;
    doc.text('Action', aCol1, doc.y, { continued: false });
    const ahY = doc.y - 9;
    doc.text('Details', aCol2, ahY);
    doc.text('Timestamp', aCol3, ahY);
    doc.moveDown(0.2);
    doc.moveTo(aCol1, doc.y).lineTo(aCol1 + pageWidth, doc.y).lineWidth(0.5).stroke();
    doc.moveDown(0.3);

    doc.font('Helvetica').fontSize(8);
    for (const entry of passAuditEntries) {
      const y = doc.y;
      let action = entry.entity_type.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
      if (entry.entity_type === 'pass_reopened') action = 'Pass Reopened';
      if (entry.entity_type === 'pass_deleted') action = 'Pass Deleted';
      if (entry.entity_type === 'round_reverse_finalize') action = 'Round Finalization Reversed';
      doc.text(action, aCol1, y);
      doc.text(entry.changed_by || '-', aCol2, y, { width: 110 });
      doc.text(entry.started_at ? new Date(entry.started_at).toLocaleString() : '-', aCol3, y);
      doc.moveDown(0.3);
  ...
  // === 7. USED SERIAL NUMBERS (Counted Ballots with Vote) ===
  doc.addPage();
  doc.fontSize(13).font('Helvetica-Bold').text('Counted Ballots');
  doc.moveDown(0.3);
  doc.fontSize(9).font('Helvetica')
